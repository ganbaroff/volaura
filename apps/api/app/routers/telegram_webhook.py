...
def _detect_ceo_emotion(message: str) -> dict:
    """ZenBrain keyword-based CEO emotion detection. No external deps."""
    msg = message.lower()
    positive = ["шикарно", "круто", "офигенно", "молодец", "заебись", "красава", "))))", "правильно"]
    negative = ["блять", "бля", "опять", "заебал", "нахуя", "проебал", "бесит", "нахрен"]
    exhausted = ["нууууу", "устал", "ну зачем"]
    pos = sum(1 for w in positive if w in msg)
    neg = sum(1 for w in negative if w in msg)
    exh = sum(1 for w in exhausted if w in msg)
    if pos > neg and pos > 0:
        state, intensity = "drive", min(5, pos * 2)
    elif neg > pos and neg > 0:
        state, intensity = "correcting", min(5, neg * 2)
    elif exh > 0:
        state, intensity = "exhausted", min(5, exh * 2)
    elif len(message) > 200:
        state, intensity = "drive", 2
    else:
        state, intensity = "analytical", 0
...
async def _classify_and_respond(db, text: str, chat_id: int | str) -> None:
    ...
    # ── Emotional scoring (ZenBrain keyword-based, no external deps) ──
    _ceo_emotion = _detect_ceo_emotion(text)
    _emotion_directive = {
        "drive": "CEO на драйве. Матчь энергию. Коротко. Действуй.",
        "warm": "CEO тёплый, открытый. Storytelling. Глубже.",
        "correcting": "CEO поправляет. Назови ошибку. Исправь. Не защищайся.",
...
async def _handle_backlog(db, chat_id: int | str) -> None:
    ...
    try:
        ...
        logger.warning("backlog read failed: {e}", e=str(e)[:100])
...
async def _handle_proposals(db, chat_id: int | str) -> None:
    ...
    try:
        ...
        logger.warning("proposals read failed: {e}", e=str(e)[:100])
...
async def _handle_proposal_action(db, chat_id: int | str, action: str, proposal_id: str) -> None:
    ...
    try:
        ...
        logger.warning("queue task failed: {e}", e=str(e)[:100])
        await _send_message(chat_id, "⚠️ Не удалось выполнить команду. Попробуй ещё раз через минуту.")
...
async def _execute_proposal(db, chat_id: int | str, proposal_id: str) -> None:
    ...
    try:
        ...
        logger.warning("proposal execute failed: {e}", e=str(e)[:150])
...
async def _trigger_autonomous_work(chat_id: int | str) -> None:
    ...
    try:
        ...
        logger.warning("autonomous trigger failed: {e}", e=str(e)[:150])
        await _send_message(chat_id, "⚠️ Не удалось запустить автономный режим. Попробуй ещё раз через минуту.")
...
- На русском""",
        ...
        logger.warning("agent task failed: {e}", e=str(e)[:100])
        reply = f"⚠️ Агент `{normalized}` временно недоступен. Попробуй ещё раз через минуту."
...
async def _handle_queue(chat_id: int | str) -> None:
    ...
    try:
        ...
        logger.warning("queue read failed: {e}", e=str(e)[:100])
...
Максимум 300 слов. Конкретно. На русском.""",
            ...
            logger.warning("swarm agent {agent_id} failed: {e}", agent_id=agent_id, e=str(e)[:80])
            responses.append(f"⚠️ {agent_id}: временно недоступен")
...
Если вопрос вне твоей экспертизы — скажи какого агента спросить.""",
        ...
        logger.warning("ask agent failed: {e}", e=str(e)[:100])
        reply = f"⚠️ Агент `{agent_name}` временно недоступен. Попробуй ещё раз через минуту."
...
Если вопрос требует кода или файлов — скажи CEO что нужно запустить сессию CTO.""",
        ...
        logger.warning("ask proposal failed: {e}", e=str(e)[:100])
        reply = "⚠️ Не смог ответить по proposal. Попробуй ещё раз через минуту."
...
async def _handle_findings(chat_id: int | str, limit: int = 5) -> None:
    ...
    try:
        ...
        logger.warning("findings read failed: {e}", e=str(e)[:100])
...
async def _handle_simulate(chat_id: int | str) -> None:
    ...
    try:
        ...
        logger.warning("simulation failed: {e}", e=str(e)[:150])
...
async def _handle_proposal_card_callback(
    ...
    if action == "details":
        ...
        try:
            ...
            logger.warning("proposal details failed: {e}", e=str(e)[:100])
            await _send_message(chat_id, "⚠️ Не удалось показать details. Попробуй ещё раз через минуту.")
        return
    ...
    try:
        ...
        logger.warning("proposals json unreadable: {e}", e=str(e)[:100])
        await _send_message(chat_id, "⚠️ Не удалось прочитать proposals. Попробуй ещё раз через минуту.")
        return
    ...
    try:
        ...
        if os.path.exists(tmp_path):
            with contextlib.suppress(Exception):
                os.remove(tmp_path)
        logger.warning("proposals write failed: {e}", e=str(e)[:100])
        await _send_message(chat_id, "⚠️ Не удалось сохранить proposals. Попробуй ещё раз через минуту.")
        return
