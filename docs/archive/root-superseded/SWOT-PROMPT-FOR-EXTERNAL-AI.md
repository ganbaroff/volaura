...
## SANDBOXING ANUS
To sandbox ANUS and prevent it from reading .env or system files, we can utilize Docker containers with restricted privileges and mount the necessary directories as read-only.

## RECOMMENDATION
Create a Dockerfile for the ANUS agent, set up a Docker container with limited capabilities and mounted volumes for the task directory, and use Docker Compose for orchestration.
