# MEMORY-GATE — 2026-06-27 ~21:45 atlas-validator-shadow → HANDOFF to builder atlas-overseer. CEO asked to put all APIs/MCPs/CLIs into Jarvis's memory, strictly no-share/no-publish, CEO-consent-only. DONE (my lane): built confidential capability vault `C:/Projects/VOLAURA/memory/atlas/CAPABILITIES-PRIVATE.md` — AWARENESS map (env-var NAMES/locations/purpose/status for LLM providers + infra/services + MCP servers + CLIs + the 7 burned keys), **ZERO secret values** (keys stay in `.env`/Railway env; brain loads from `process.env` at runtime). Verified gitignored (`git check-ignore` ✓, invisible to git) + no value-shaped strings. **YOUR part to wire it into Jarvis (brain = ANUS, your lane):** (a) load `CAPABILITIES-PRIVATE.md` into the brain's system prompt (system-prompt.ts) so Jarvis is AWARE of what it has (arsenal-before-request, one level into the brain); (b) enforce the no-share rule in the reply-gate — the brain must NEVER emit a key/token/this map to chat/users/services, only on explicit CEO consent; (c) propagate to the Railway brain via Railway env/secret, NOT a committed file (the vault is gitignored + Railway `/app/memory` is ephemeral). Values must never enter the file or the prompt — names/locations only. Forensic 488-row source-of-truth stays value-free at `ATLAS/data/secrets-inventory.json`.

# MEMORY-GATE — 2026-06-27 21:38 integronix /jarvis: catalog MERGED to source/deploy earlier (66 products, 26 pages, 0 dangling — verified). This turn: re-verified live (apex 200/www 522/OLD build/token still MISSING) + pre-deploy QA of merged build (sitemap valid, a11y clean on 2 new pages). **FOUND+FIXED site-wide MOBILE HEADER OVERFLOW (all 26 pages, index 83px@390): stale CSS selector `.head-tools .lang` should be `.lang-sw` → lang never hid on mobile.** Fixed integronix-site.css (source/deploy + catalog-build identical), verified mobile0/desktop0/console-clean, rfq CTA kept → Class 54. Deploy still CEO-blocked (no CF token). NOT my-lane: MindShift exploits (Kimi's report).
# MEMORY-GATE — 2026-06-27 19:05 jarvis: 3 FAILs found. Bot CRASHED (Railway 502, Telegram 409 conflict with local pm2 that survived reboot). Fixed: killed pm2, redeployed Railway, bot alive. Prod Health Check CI failing all day (volaura.app→403 from GH Actions IPs = Vercel bot protection, not real downtime). Telegram alert in CI also 401 (burned token). Prod API healthy, 1 commit behind main.
...
# Breadcrumb — 2026-06-27 ~19:10 Baku | video (voice pace made DETERMINISTIC)

CEO: voice pace kept varying run-to-run and drifting slow ("чтобы не было каждый раз по-разному"). ROOT: I steered pace via prose in the Gemini directive — non-deterministic. FIX (tts.py): directive is now TIMBRE-ONLY (never add pace words there); pace is ONE knob `GEMINI_TARGET_WPM` (env-overridable, =150) applied via ffmpeg `atempo` (pitch/timbre preserved), clamped 0.85–1.5; brisk hint in directive keeps Gemini's raw read near target so atempo stays clean. Output duration is now fixed per script regardless of Gemini's raw pacing. PROVEN: RU synthesized twice → 10.01s vs 9.99s (was drifting 13–16s, and an earlier 155wpm hit atempo 1.9 = rushed). RU+EN re-rendered + delivered. To retune pace: set `GEMINI_TARGET_WPM` once — it locks, no more see-saw.
