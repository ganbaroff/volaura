...
def _read_project_state(project_root: Path) -> str:
    ...
    # This is the NEVER_PROPOSE rules + HIGH_VALUE patterns + OPEN_FINDINGS
    # Generated by memory_consolidation.py (sleep cycle daemon)
    ...
    if distilled_file.exists():
        with open(distilled_file, "r", encoding="utf-8") as f:
            content = f.read()
        # Inject full distilled file — it's already compressed by LLM synthesis
        state_parts.append("## DISTILLED AGENT KNOWLEDGE (read carefully — hard rules + open findings)\n" + content)
    else:
        # Fallback: if distilled doesn't exist yet, try raw log (first 3000 chars only)
        feedback_log = project_root / "memory" / "swarm" / "agent-feedback-log.md"
        if feedback_log.exists():
            with open(feedback_log, "r", encoding="utf-8") as f:
                lines = f.readlines()[-60:]  # Last 60 lines = most recent entries
            state_parts.append("## RECENT AGENT FEEDBACK (raw — distilled file not yet generated)\n" + "".join(lines))
