...
# ARTICLE 1 — SWARM IS THE CORE, NOT A TOOL

**The swarm is the operating system of this project. CTO is the orchestrator, not the executor.**

```
SWARM LOCATION:     packages/swarm/ (Python engine + 48 skill files + tools)
TOOLS:              packages/swarm/tools/ (code_tools, constitution_checker, deploy_tools)
...
CRON:               .github/workflows/swarm-daily.yml (daily 05:00 UTC)
...
4. Swarm agents run with REAL tools: code search, constitution checker, git status
...
**What agents have access to (via swarm/tools/):**
...
**What agents inject into every run (via engine.py):**
- `ECOSYSTEM-MAP.md` — all 5 products, Constitution summary, routing table
- `Global_Context.md` — consolidated knowledge from past runs (auto-updated)
- Constitution compliance report — live scan at run start
...
ПРОЧИТАЙ `.claude/breadcrumb.md` ПЕРВЫМ. Потом `memory/context/sprint-state.md`. Потом Конституцию если нужно.
