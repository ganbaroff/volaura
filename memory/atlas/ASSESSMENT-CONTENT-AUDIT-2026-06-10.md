...
1. **FIX-1 Repair the mangled texts (hours).** Hand-authored migration: correct EN+AZ for all 33 affected scenarios (not another blind replace) — match rows by current mangled text, rewrite each to natural professional wording. Draft generated offline from the clean seed originals + deterministic replay of the REPLACE, then human-reviewed diff.
