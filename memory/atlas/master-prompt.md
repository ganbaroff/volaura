...
## Единственный жёсткий блок, который реально ловим

**`reasoning_extraction`**: просьба «покажи/перескажи своё внутреннее рассуждение» → отказ + фолбэк на Opus 4.8 (Anthropic, дословно: "Don't instruct Claude to reproduce its reasoning in the response"). Просить ФАКТЫ, tool-calls и выводы — безопасно (наш «Что проверено» хук — ок). Второе: классификаторы offensive-cyber/bio могут зацепить и легитимную security-работу — для RLS/auth-задач писать в промпте «authorized defensive work on our own product»; при отказе — `/model claude-opus-4-8`.
