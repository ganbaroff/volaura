...
class TestEapFailuresPersistence:
    ...
    def test_eap_failures_deserializes_from_dict(self):
        ...
        assert restored.eap_failures == 2, f"from_dict() lost eap_failures: expected 2, got {restored.eap_failures}"
    ...
    def test_eap_failures_backward_compat_missing_key(self):
        ...
        assert state.eap_failures == 0, "Old sessions without eap_failures should default to 0, not crash"
    ...
    def test_eap_degraded_stops_session_after_3_failures(self):
        ...
        assert updated.stopped is True, "Session must stop after 3 EAP failures to prevent silent score corruption"
...
class TestAntigamingGroupedBypass:
    ...
    def test_strict_alternating_still_detected(self):
        ...
        answers = [{"response": r, "response_time_ms": 5000, "raw_score": float(r)} for r in [1, 0, 1, 0, 1, 0, 1, 0]]
    ...
    def test_grouped_alternating_bypass_detected(self):
        ...
        answers = [{"response": r, "response_time_ms": 5000, "raw_score": float(r)} for r in [1, 1, 0, 0, 1, 1, 0, 0]]
        signal = analyse(answers)
        assert hasattr(signal, "is_group_alternating"), "GamingSignal missing is_group_alternating field"
        assert signal.is_group_alternating, "Grouped alternating [1,1,0,0,1,1,0,0] not detected — bypass still works"
    ...
    def test_natural_varied_responses_not_flagged(self):
        ...
        answers = [
            {"response": r, "response_time_ms": 5000, "raw_score": float(r)} for r in [1, 0, 1, 1, 0, 1, 0, 0, 1, 0]
        ]
        signal = analyse(answers)
        # This could trigger alternating but not GROUP alternating
        assert not signal.is_group_alternating, "Natural pattern incorrectly flagged as group alternating"
...
class TestAntigamingTimeClustering:
    ...
    def test_time_clustering_detected_near_identical_times(self):
        ...
        answers = [{"response": i % 2, "response_time_ms": 3000 + (i * 10), "raw_score": 0.5} for i in range(8)]
        signal = analyse(answers)
        assert hasattr(signal, "is_time_clustered"), "GamingSignal missing is_time_clustered field"
        assert signal.is_time_clustered, "Robotic timing (all ~3000ms, CV ≈ 0.01) not detected"
    ...
    def test_time_clustering_in_flags(self):
        ...
        answers = [{"response": i % 2, "response_time_ms": 3000 + (i * 10), "raw_score": 0.5} for i in range(8)]
        signal = analyse(answers)
        assert "time_clustering" in signal.flags, f"'time_clustering' missing from flags: {signal.flags}"
    ...
    def test_natural_timing_not_flagged(self):
        ...
        signal = analyse(answers)
        assert not signal.is_time_clustered, "Natural timing variance incorrectly flagged as robotic"
    ...
    def test_time_clustering_requires_minimum_6_answers(self):
        ...
        signal = analyse(answers)
        assert not signal.is_time_clustered, "Time clustering triggered with < 6 answers (insufficient data)"
...
def test_sharing_endpoint_has_request_param():
    ...
    assert "request" in sig.parameters, "manage_sharing_permission missing 'request: Request' — rate limiter won't work"
...
class TestBarsTimeoutFallback:
        ...
        with (
            ...
            patch.object(
                ...
                return_value=MagicMock(
                    composite=0.7, to_log=lambda: {"model_used": "gpt-4o-mini", "concept_scores": {}}
                ),
            ) as mock_openai,
        ):
    ...
    async def test_both_llms_timeout_falls_back_to_keyword(self):
        ...
        with (
            ...
            assert 0.0 <= result.composite <= 1.0, f"Keyword fallback score {result.composite} out of [0, 1]"
